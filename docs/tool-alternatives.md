...
Current defaults and optional installation groups are documented in
[package profiles](package-profiles.md). References below to the Brewfile
include those optional profiles.
...
Mise uses the explicit npm backend with the pinned Node runtime. The selected
backend is a consistent installation choice; no claim is made that the current
aqua backend is broken. pnpm 11 settings live in the managed
`.config/pnpm/config.yaml`, and project `packageManager` pins remain
authoritative.
...
| Codex CLI                                        | The installer uses OpenAI's recommended standalone installer, which installs to `~/.local/bin` and updates independently of Homebrew.                         |
| Claude Code                                      | The installer uses Anthropic's recommended native installer, which installs to `~/.local/bin` and updates automatically.                                      |
...
`terraform`. To switch, replace the Brewfile.cloud line with `brew "opentofu"`
and use `tofu` in place of `terraform`.
